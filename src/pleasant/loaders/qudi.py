...
def load_qudi_folder(folder_name, description_contains='', break_duration=None):
    ...
    """
    filename_stubs = [i for i in _get_filename_stubs(folder_name) if description_contains in i]
    if not filename_stubs:
        print('No file containing the description {} found.'.format(description_contains))
        return []
    ...
    for filename_stub in filename_stubs:
        ...
        for which in ['trace', 'retrace']:
            rate, exc_freq, scan_duration = _read_data_files(filename_stub_with_folder, which=which)
            _m = Measurement(rate, exc_freq, timestamp=timestamp, description=description, scan_duration=scan_duration,
...
def _get_filename_stubs(folder_name):
    ...
    filename_stubs = [i.removesuffix(suffix) for i in listdir(folder_name) if i.endswith(suffix)]
...
def _get_scan_params(filename):
    ...
    p_dict = {'v_start': float(header[6].split(':')[1]),
...
def _read_data_files(filename_stub, which='trace'):
    ...
    scan_duration = abs(scan_params['v_start'] - scan_params['v_stop']) / scan_params['v_speed']
